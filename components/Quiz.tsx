  ...
  const fetchQuestion = async () => {
    ...
    try {
      ...
      if (data && data.question) {
        ...
        setError("Failed to generate a valid question. Retrying...");
        // Auto retry once
        setTimeout(fetchQuestion, 2000);
      }
    } catch (e) {
      setError("The assessment engine is currently offline.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 bg-slate-900 rounded-2xl border border-slate-800">
        ...
        <p className="text-slate-400 font-medium animate-pulse">Generating engineering challenge...</p>
      </div>
    );
  }
  ...
  if (error && !question) {
    return (
      <div className="flex flex-col items-center justify-center h-96 bg-slate-900 rounded-2xl border border-slate-800 p-8 text-center">
        <div className="text-red-500 mb-4 font-bold">⚠️ Assessment Error</div>
        <p className="text-slate-400 mb-6">{error}</p>
        <button onClick={fetchQuestion} className="px-6 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-bold">
          Reconnect to Lab
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-slate-900 rounded-2xl border border-slate-800 p-8 shadow-xl max-w-2xl mx-auto transition-all">
      <div className="flex justify-between items-center mb-6">
        <span className="bg-blue-600/20 text-blue-400 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Engineering Assessment</span>
        <span className="text-slate-400 font-mono text-sm">SCORE: {score * 10} pts</span>
      </div>
        ...
        <div className="animate-in fade-in slide-in-from-top-4 duration-500">
          <div className={`p-6 rounded-xl border mb-6 ${isCorrect ? 'bg-green-950/20 border-green-500/30' : 'bg-red-950/20 border-red-500/30'}`}>
            <h4 className={`font-bold mb-2 ${isCorrect ? 'text-green-400' : 'text-red-400'}`}>
              {isCorrect ? 'Correct Analysis' : 'Vulnerability Missed'}
            </h4>
